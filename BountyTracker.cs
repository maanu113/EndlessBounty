...
{
    /// <summary>
    /// Core reward logic: while the final wave is active (tracked via
    /// BountyMobs' kill feed as a live signal), periodically spawns a free
    /// chest / elite-table item / building-upgrade pickup, paced by TIME
    /// survived rather than kill count - kill rate in the final wave scales
    /// into the hundreds per minute, so any kills-based cadence explodes
    /// regardless of tuning. The game already grants coins per kill on its
    /// own; this mod does not duplicate that anymore.
    ...
    internal sealed class BountyTracker
    {
        ...
        /// <summary>Advances the survival timers while the final wave is active. A kill in the last few seconds is used as a cheap "final wave is currently active" signal.</summary>
        public void Update()
        {
            ...
            bool finalWaveActive = BountyMobs.TrackedAliveCount > 0 || Time.time - LastKillTime < 5f;
            if (!finalWaveActive)
            {
                return;
            }
